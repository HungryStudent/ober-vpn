...
async def new_device_start(call: CallbackQuery, state: FSMContext):
  ...
  WireGuard: Не имеет ограничений по трафику. Однако он может быть заблокирован в некоторых регионах при использовании мобильного интернета или WiFi. Один конфиг-файл может быть использован только на одном устройстве.
...
Для обеспечения максимальной доступности рекомендуем использовать Outline, так как при использовании WireGuard возможны проблемы с блокировками.""",
